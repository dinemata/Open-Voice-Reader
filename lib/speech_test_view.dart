...
class _SpeechTestViewState extends State<SpeechTestView> {
  ...
  final ValueNotifier<HighlightData> _highlightNotifier =
      ValueNotifier<HighlightData>(HighlightData(sentenceRects: []));
  ...
  double get _currentZoomFactor =>
      globalIsOriginalLayout ? _pdfZoomFactor : _textZoomFactor;
  ...
  void initState() {
    ...
    if (Platform.isAndroid || Platform.isIOS) {
      ...
      _positionSubscription = audioHandler.player.positionStream.listen(
        (p) => _updateWordHighlight(p),
      );
    } else {
      _playbackStateSubscription = windowsPlayer.processingStateStream.listen((
        state,
      ) {
        if (!mounted) return;
      ...
      _positionSubscription = windowsPlayer.positionStream.listen(
        (p) => _updateWordHighlight(p),
      );
    }
  }
  ...
  void _scrollListener() {
    if (_chunksMetadata.isEmpty ||
        ...
        globalIsOriginalLayout)
      return;
    final positions = _itemPositionsListener.itemPositions.value;
    if (positions.isEmpty) return;
    bool currentVisible = positions.any(
      (position) => position.index == _currentChunkIndex,
    );
  ...
  Future<void> _selectInitialModel(ModelConfig model) async {
    ...
    if (booksJson != null) {
      ...
      final List<BookModel> list = decoded
      ...
      if (_isTxtFile) {
        ...
        _parseTxtWithProgress(content).listen(
          (progressValue) {
            if (!mounted) return;
            setState(() {
              _parsingProgress = progressValue;
            });
          },
          onDone: () {
            if (!mounted) return;
            setState(() {
              ...
              if (_chunksMetadata.isEmpty) {
                _chunksMetadata.add(
                  PdfChunkMetadata(
                    text: "Soubor neobsahuje žádný text.",
                    pageNumber: 1,
                    pdfWords: [],
                  ),
                );
        ...
        TextSanitizer.parsePdfWithProgress(
          ...
          onChunkReady: () {
            if (mounted &&
                _chunksMetadata.isNotEmpty &&
                _currentChunkIndex < _chunksMetadata.length) {
              setState(() {
                globalCurrentPdfPage =
                    _chunksMetadata[_currentChunkIndex].pageNumber;
              });
            }
          },
        ).listen(
          ...
          onDone: () {
            ...
            setState(() {
              ...
              if (_chunksMetadata.isEmpty) {
                _chunksMetadata.add(
                  PdfChunkMetadata(
                    text: "PDF neobsahuje žádný text.",
                    pageNumber: 1,
                    pdfWords: [],
                  ),
                );
  ...
  Stream<double> _parseTxtWithProgress(String content) async* {
    ...
    final sanitizedContent = TextSanitizer.sanitizeText(
      content,
      _sanitizerOptions,
    );
    final List<String> rawSentences = sanitizedContent.split(
      RegExp(r'(?<=[.!?])\s+'),
    );
    ...
    for (int i = 0; i < total; i++) {
      ...
      if (clean.isNotEmpty) {
        _chunksMetadata.add(
          PdfChunkMetadata(text: clean, pageNumber: 1, pdfWords: []),
        );
      }
      if (i % 5 == 0 || i == total - 1) {
        yield (i + 1) / total;
  ...
  void _updateWordHighlight(Duration position) {
    ...
    if (calculatedWordIndex != _currentWordIndex &&
        calculatedWordIndex < words.length) {
      setState(() {
  ...
  Future<void> _initEngine() async {
    try {
      ...
      await _prepareFile(
        '$esAssetDir/phondata-manifest',
        targetPath: '$esSub/phondata-manifest',
      );
      await _prepareFile(
        '$esAssetDir/phonindex',
        targetPath: '$esSub/phonindex',
      );
      await _prepareFile(
        '$esAssetDir/intonations',
        targetPath: '$esSub/intonations',
      );

      if (_selectedModel.id == 'en_alan') {
        await _prepareFile(
          'assets/models/vits-piper-en_GB-alan-medium/espeak-ng-data/en_dict',
          targetPath: '$esSub/en_dict',
        );
      } else {
      ...
      await _prepareFile(
        'assets/models/vits-piper-cs_CZ-jirka-medium/espeak-ng-data/cs_dict',
        targetPath: '$esSub/cs_dict',
      );
      await _prepareFile(
        '$esAssetDir/lang/gmw/en',
        targetPath: '$esSub/lang/gmw/en',
      );
      await _prepareFile(
        '$esAssetDir/lang/gmw/en-US',
        targetPath: '$esSub/lang/gmw/en-US',
      );
      ...
      await File(
        '${voicesDir.path}/cs',
      ).writeAsString("name cs\nlanguage cs\n");
      await File(
        '${voicesDir.path}/en-us',
      ).writeAsString("name en-us\nlanguage en-us\n");

      final espeakDataPath = '${directory.path}/$esSub'.replaceAll('\\', '/');
      sherpa.OfflineTtsModelConfig modelConfig;

      final modelPath = await _prepareFile(
        '${_selectedModel.assetDir}/${_selectedModel.modelFile}',
        targetPath: '${_selectedModel.id}_model.onnx',
      );
      final tokensPath = await _prepareFile(
        '${_selectedModel.assetDir}/tokens.txt',
        targetPath: '${_selectedModel.id}_tokens.txt',
      );
      ...
      if (_selectedModel.id == 'en_kokoro') {
        final extraPath = await _prepareFile(
          '${_selectedModel.assetDir}/${_selectedModel.configFile}',
          targetPath: _selectedModel.configFile,
        );
        ...
        await _prepareFile(
          '${_selectedModel.assetDir}/${_selectedModel.configFile}',
          targetPath: '${_selectedModel.id}_config.json',
        );
        modelConfig = sherpa.OfflineTtsModelConfig(
  ...
  void _updatePdfVisualHighlights() {
    ...
    final double scaleFactor =
        (_pdfViewerSize.width / nativePage.size.width) *
        (_pdfZoomFactor < 1.0 ? 1.0 : _pdfZoomFactor);
  ...
  void _handlePdfTap(TapUpDetails d) {
    ...
    final Offset pdfPointsPos = Offset(
      actualX / scaleFactor,
      actualY / scaleFactor,
    );
  ...
  Future<void> _bufferNextChunkAsync(int nextIndex) async {
    if (nextIndex >= _chunksMetadata.length ||
        _pregeneratedAudioCache.containsKey(nextIndex) ||
        _isBufferingNext)
      return;
    ...
    try {
      ...
      final wavPath = p.join(
        tempDir.path,
        'chunk_${nextIndex}_${DateTime.now().millisecondsSinceEpoch}.wav',
      );
      if (sherpa.writeWave(
        filename: wavPath,
  ...
  void _jumpToSelectedAndPlay() async {
    if (_selectedChunkIndex == null ||
        _selectedChunkIndex! >= _chunksMetadata.length)
      return;
    setState(() {
      _isBusy = true;
      _isUserScrolling = false;
  ...
  void _executeChunkReading() async {
    ...
    try {
      ...
      if (wavPath == null) {
        if (rawText.trim().isEmpty || _tts == null) {
  ...
  void _changeZoom(double delta) {
    setState(() {
      if (globalIsOriginalLayout) {
        ...
        _pdfViewerController.zoomLevel = _pdfZoomFactor < 1.0
            ? 1.0
            : _pdfZoomFactor;
    ...
    return Row(
      ...
      children: List.generate(3, (i) {
        ...
        if (i < load)
          color = load == 1
              ? Colors.green
              : (load == 2 ? Colors.orange : Colors.red);
        return Container(
          width: 3,
          height: 10,
  ...
  Widget build(BuildContext context) {
    bool isCompleted =
        _chunksMetadata.isNotEmpty &&
        _currentChunkIndex >= _chunksMetadata.length;
    double progress = _chunksMetadata.isEmpty
        ? 0.0
        : (isCompleted ? 1.0 : _currentChunkIndex / _chunksMetadata.length);
                        ...
                        Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<ModelConfig>(
                              ...
                              onChanged: !_isBusy && _isReady
                                  ? (ModelConfig? newValue) {
                                      if (newValue != null) {
                                        setState(() {
                        ...
                        if (!_isTxtFile)
                          SizedBox(
                            ...
                            child: TextButton.icon(
                              ...
                              label: Text(
                                globalIsOriginalLayout ? 'PDF' : 'Text',
                                style: const TextStyle(
                                  fontSize: 12,
                                  fontWeight: FontWeight.w600,
                                ),
                        ...
                        onSelected: (value) async {
                          setState(() {
                            if (value == 'parentheses') {
                              _sanitizerOptions.readParentheses =
                                  !_sanitizerOptions.readParentheses;
                            } else if (value == 'links') {
                              _sanitizerOptions.readLinks =
                                  !_sanitizerOptions.readLinks;
                            } else if (value == 'pages') {
                              _sanitizerOptions.readPageNumbers =
                                  !_sanitizerOptions.readPageNumbers;
                            }
                          });
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const Text(
                            'Vyberte hlas pro tento dokument',
                            style: TextStyle(
                              fontSize: 18,
                              fontWeight: FontWeight.w800,
                              color: Color(0xFF1F1F1F),
                          ...
                          const Text(
                            'Tento dokument nemá přiřazený model. Zvolte výchozí formát čtení:',
                                                      ...
                                                      child:
                                                          ...
                                                          ? RichText(
                                                              text: TextSpan(
                                                                ...
                                                                children:
                                                                    _buildHighlightedWords(
                                                                      _chunksMetadata[index]
                                                                          .text,
                                                                      context,
                                                                    ),
                                                              ),
